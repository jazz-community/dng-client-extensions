/*
 * Change link type of selected artifacts.
 *
 * Donat Hutter, +41 41 724-3531, donat.hutter@siemens.com
...
function processAttributes(refs) {
    ...
    if (testMode) {
        info("Test " + progressMax + " artifacts ...");
    } else {
...
function changeLinks(id, linkset) {
    if (testMode) {
        linkset.forEach(function(linkDefinition) {
            ...
            countLink += linkDefinition.targets.length;
        });
        progressDone++;
    } else {
        linkset.forEach(function(linkDefinition) {
...
function progressBarShow(v) {
    ...
    if (progressDone >= progressMax) {
        ...
        if (testMode) {
            if (countErr > 0) {
                finalMessage.innerHTML = countLink + " links ... check log for errors.";
            } else {
                finalMessage.innerHTML = countLink + " links.";
            }
        } else {
            if (countErr > 0) {
